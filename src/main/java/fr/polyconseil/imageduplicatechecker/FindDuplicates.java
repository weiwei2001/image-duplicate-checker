...
public class FindDuplicates {
	...
	public static void find(Map<String, List<String>> duplicatesMap, File directory, boolean leanAlgorithm)
			throws Exception {
	...
	/*
	 * quick but memory hungry (might like to run with java -Xmx2G or the like
	 * to increase heap space if RAM available)
	 */
	...
	/*
	 * slower but memory efficient -- you might like to play with the size
	 * defined by "buffSize"
	 */
	...
	public static void execute(String excludedFolder, String allowedFileExtensions, String sourcePathPattern,
		...
		// Wait for Java 8:
		// duplicatesMap.keySet().removeIf(e->(duplicatesMap.get(e).size()<2));
		Map<String, List<String>> uniqueDuplicatesMap = new HashMap<String, List<String>>();
		for (String key : duplicatesMap.keySet()) {
			if (duplicatesMap.get(key).size() > 1) {
				uniqueDuplicatesMap.put(key, duplicatesMap.get(key));
			}
		}
		generateReport(sourcePathPattern, workspace, uniqueDuplicatesMap);
		listener.getLogger().println(uniqueDuplicatesMap.size() + " results generated in file: " + sourcePathPattern);
	}

	private static void generateReport(String sourcePathPattern, FilePath workspace,
			Map<String, List<String>> duplicatesMap) {
		FilePath f = workspace.child(sourcePathPattern);
		PrintWriter w;
		try {
			w = new PrintWriter(new BufferedWriter(new OutputStreamWriter(f.write(), "UTF-8")));
			...
			throw new RuntimeException("cannot read unsupported encoding in " + sourcePathPattern, e);
		} catch (IOException e) {
			throw new RuntimeException("cannot read file " + sourcePathPattern + " in " + workspace.getRemote(), e);
		} catch (InterruptedException e) {
			throw new RuntimeException("cannot write file " + sourcePathPattern + " in " + workspace.getRemote(), e);
		}
	}
}
